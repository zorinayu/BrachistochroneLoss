#!/usr/bin/env python3
"""
Test BRACHISTOCHRONE method on NSL-KDD dataset
"""
...
def get_args():
    parser = argparse.ArgumentParser(description='Test BRACHISTOCHRONE on NSL-KDD dataset')
    parser.add_argument('--sample_size', type=int, default=10000, help='Sample size for testing')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
    parser.add_argument('--output_dir', type=str, default='outputs/nslkdd', help='Output directory')
    return parser.parse_args()

class NSLKDDDataset(torch.utils.data.Dataset):
    def __init__(self, train_path='data/NSL-KDD dataset/KDDTrain+.txt', 
                 test_path='data/NSL-KDD dataset/KDDTest+.txt', 
        ...
        # Define column names (simplified)
        ...
        print(f"NSL-KDD Dataset: {len(self.X_train)} train, {len(self.X_test)} test")
        print(f"Features: {self.n_features}, Classes: {self.n_classes}")
        print(f"Attack rate: {(y_encoded == 1).mean():.4f}")
...
def test_nslkdd_dataset(sample_size, epochs, batch_size, device, output_dir):
    """Test all methods on NSL-KDD dataset"""
    print("=" * 60)
    print("TESTING NSL-KDD DATASET")
    print("=" * 60)
    
    # Load dataset
    dataset = NSLKDDDataset(sample_size=sample_size)
...
def save_numerical_summary(results, output_dir):
    ...
    with open(os.path.join(output_dir, 'nslkdd_results.txt'), 'w') as f:
        f.write("BRACHISTOCHRONE METHOD TEST RESULTS - NSL-KDD DATASET\n")
        f.write("=" * 60 + "\n\n")
