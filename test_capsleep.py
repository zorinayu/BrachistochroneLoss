#!/usr/bin/env python3
"""
Test BRACHISTOCHRONE method on CAP Sleep Database dataset
"""
...
def get_args():
    parser = argparse.ArgumentParser(description='Test BRACHISTOCHRONE on CAP Sleep dataset')
    parser.add_argument('--sample_size', type=int, default=5000, help='Sample size for testing')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')
    parser.add_argument('--output_dir', type=str, default='outputs/capsleep', help='Output directory')
    return parser.parse_args()

class CAPSleepDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir='data/CAP Sleep Database/Balanced Data/Balanced Data', 
        ...
        for label, filename in files.items():
            ...
            if os.path.exists(filepath):
                ...
                # Read only first few rows due to large file size
                df = pd.read_csv(filepath, nrows=sample_size//len(files) if sample_size else 1000)
        ...
        if not all_data:
            raise ValueError("No data loaded from CAP Sleep Database")
        ...
        print(f"CAP Sleep Dataset: {len(self.X_train)} train, {len(self.X_test)} test")
...
def test_capsleep_dataset(sample_size, epochs, batch_size, device, output_dir):
    """Test all methods on CAP Sleep dataset"""
    print("=" * 60)
    print("TESTING CAP SLEEP DATASET")
    print("=" * 60)
    
    # Load dataset
    dataset = CAPSleepDataset(sample_size=sample_size)
...
def save_numerical_summary(results, output_dir):
    ...
    with open(os.path.join(output_dir, 'capsleep_results.txt'), 'w') as f:
        f.write("BRACHISTOCHRONE METHOD TEST RESULTS - CAP SLEEP DATASET\n")
        f.write("=" * 60 + "\n\n")
