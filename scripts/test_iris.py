#!/usr/bin/env python3
"""
Test BRACHISTOCHRONE method on Iris dataset
"""
...
def get_args():
    parser = argparse.ArgumentParser(description='Test BRACHISTOCHRONE on Iris dataset')
    parser.add_argument('--sample_size', type=int, default=150, help='Sample size for testing')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')
    parser.add_argument('--output_dir', type=str, default='../outputs/iris', help='Output directory')
    return parser.parse_args()

class IrisDataset(torch.utils.data.Dataset):
    def __init__(self, data_path='data/iris.zip', sample_size=None, test_size=0.2, random_state=42):
        
        print("Loading Iris dataset...")
        
        # Load Iris data from sklearn (more reliable than zip file)
        iris = load_iris()
        ...
        print(f"Iris Dataset: {len(self.X_train)} train, {len(self.X_test)} test")
...
def test_iris_dataset(sample_size, epochs, batch_size, device, output_dir):
    """Test all methods on Iris dataset"""
    print("=" * 60)
    print("TESTING IRIS DATASET")
    print("=" * 60)
    
    # Load dataset
    dataset = IrisDataset(sample_size=sample_size)
    ...
    for method_name, brach_loss in methods.items():
        ...
        if isinstance(method_epochs, list):
            ...
            for epoch_count in method_epochs:
                ...
                # Create model
                model = MLPClassifier(dataset.n_features, [32, 16], dataset.n_classes).to(device)
...
def save_numerical_summary(results, output_dir):
    ...
    with open(os.path.join(output_dir, 'iris_results.txt'), 'w') as f:
        f.write("BRACHISTOCHRONE METHOD TEST RESULTS - IRIS DATASET\n")
        f.write("=" * 60 + "\n\n")
