#!/usr/bin/env python3
"""
Test BRACHISTOCHRONE method on MNIST dataset
"""
...
def get_args():
    parser = argparse.ArgumentParser(description='Test BRACHISTOCHRONE on MNIST dataset')
    parser.add_argument('--sample_size', type=int, default=10000, help='Sample size for testing')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
    parser.add_argument('--output_dir', type=str, default='../outputs/mnist', help='Output directory')
    return parser.parse_args()

class MNISTDataset(torch.utils.data.Dataset):
    def __init__(self, data_path='data/mnist-master.zip', sample_size=None, test_size=0.2, random_state=42):
        ...
        try:
            with zipfile.ZipFile(data_path, 'r') as zip_ref:
                ...
                # Load training data
                train_files = [f for f in os.listdir(temp_dir) if f.startswith('train') and f.endswith('.csv')]
                if train_files:
                    train_file = os.path.join(temp_dir, train_files[0])
                ...
                # Load test data
                test_files = [f for f in os.listdir(temp_dir) if f.startswith('test') and f.endswith('.csv')]
                if test_files:
                    test_file = os.path.join(temp_dir, test_files[0])
            ...
            print("Using sklearn's MNIST-like dataset (digits)")
            from sklearn.datasets import load_digits
            digits = load_digits()
            X, y = digits.data, digits.target
            
            # Resize to 28x28 (pad with zeros)
            X_resized = np.zeros((len(X), 784))
            for i, img in enumerate(X):
                # Reshape 8x8 to 28x28 by padding
                img_8x8 = img.reshape(8, 8)
                img_28x28 = np.zeros((28, 28))
                img_28x28[10:18, 10:18] = img_8x8
                X_resized[i] = img_28x28.flatten()
            
            X = X_resized / 16.0  # Normalize
        ...
        print(f"MNIST Dataset: {len(self.X_train)} train, {len(self.X_test)} test")
...
def test_mnist_dataset(sample_size, epochs, batch_size, device, output_dir):
    """Test all methods on MNIST dataset"""
    print("=" * 60)
    print("TESTING MNIST DATASET")
    print("=" * 60)
    
    # Load dataset
    dataset = MNISTDataset(sample_size=sample_size)
...
def save_numerical_summary(results, output_dir):
    ...
    with open(os.path.join(output_dir, 'mnist_results.txt'), 'w') as f:
        f.write("BRACHISTOCHRONE METHOD TEST RESULTS - MNIST DATASET\n")
        f.write("=" * 60 + "\n\n")
