#!/usr/bin/env python3
"""
Test BRACHISTOCHRONE method on MovieLens dataset
"""
...
def get_args():
    parser = argparse.ArgumentParser(description='Test BRACHISTOCHRONE on MovieLens dataset')
    parser.add_argument('--sample_size', type=int, default=5000, help='Sample size for testing')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
    parser.add_argument('--output_dir', type=str, default='../outputs/movielens', help='Output directory')
    return parser.parse_args()

class MovieLensDataset(torch.utils.data.Dataset):
    def __init__(self, data_path='data/ml-32m.zip', sample_size=None, test_size=0.2, random_state=42):
        ...
        try:
            with zipfile.ZipFile(data_path, 'r') as zip_ref:
                ...
                if data_files:
                    ...
                    # Create features from user_id and movie_id
                    ...
                    X = df[['user_id_encoded', 'movie_id_encoded']].values
                    y = df['rating'].values
            ...
            print("Using synthetic MovieLens-like data")
            # Create synthetic MovieLens-like data
            ...
            # Create feature matrix
            X = np.column_stack([user_ids, movie_ids]).astype(np.float32)
            
            # Generate ratings based on user and movie preferences
            # Some users are more generous, some movies are better rated
            user_bias = np.random.normal(0, 0.5, n_users)
            movie_bias = np.random.normal(0, 0.3, n_movies)
            
            y = (3.5 +  # Base rating
                 user_bias[user_ids] +  # User bias
                 movie_bias[movie_ids] +  # Movie bias
        ...
        print(f"MovieLens Dataset: {len(self.X_train)} train, {len(self.X_test)} test")
        print(f"Features: {self.n_features}, Target: Continuous (ratings)")
...
def test_movielens_dataset(sample_size, epochs, batch_size, device, output_dir):
    """Test all methods on MovieLens dataset"""
    print("=" * 60)
    print("TESTING MOVIELENS DATASET")
    print("=" * 60)
    
    # Load dataset
    dataset = MovieLensDataset(sample_size=sample_size)
    ...
    for method_name, brach_loss in methods.items():
        ...
        if isinstance(method_epochs, list):
            ...
            for epoch_count in method_epochs:
                ...
                # Create model
                model = MLPRegressor(dataset.n_features, [128, 64], 1).to(device)
...
def save_numerical_summary(results, output_dir):
    ...
    with open(os.path.join(output_dir, 'movielens_results.txt'), 'w') as f:
        f.write("BRACHISTOCHRONE METHOD TEST RESULTS - MOVIELENS DATASET\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    ...
    # Test dataset
    results = test_movielens_dataset(
